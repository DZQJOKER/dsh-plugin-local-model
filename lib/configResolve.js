...
export function logLevelOf(value) {
    return value === 'silent' || value === 'error' || value === 'warn' || value === 'debug' ? value : 'info';
}
export function isAutoUnloadDisabled(minutes) {
...
export function resolveConfig(input, env = process.env) {
    ...
    return {
        ...
        kvmemMethod: normalizeChoice(merged.kvmemMethod, ['', 'recency', 'retrieval'], ''),
        ...
        kvmemQueryReplay: normalizeChoice(merged.kvmemQueryReplay, ['', 'legacy', 'auto'], ''),
        kvmemQueryPolicy: normalizeChoice(merged.kvmemQueryPolicy, ['', 'legacy', 'user'], ''),
        kvmemMtpState: normalizeChoice(merged.kvmemMtpState, ['', 'snapshots', 'auto', 'replay'], ''),
        ...
        loadMode: normalizeChoice(merged.loadMode, ['', 'auto', 'none', 'mmap', 'mlock', 'mmap+mlock', 'dio'], ''),
        kvDtype: normalizeChoice(merged.kvDtype, ['', 'f16', 'f32', 'q8_0', 'q5_0', 'q4_0'], ''),
        specKvDtype: normalizeChoice(merged.specKvDtype, ['', 'f16', 'f32', 'q8_0', 'q5_0', 'q4_0'], ''),
        ...
        reasoningEffort: normalizeChoice(merged.reasoningEffort, ['', 'none', 'minimal', 'low', 'medium', 'high', 'xhigh', 'max'], ''),
...
export function consistencyNotices(config) {
    ...
    if (Number.isFinite(ctxSize) && Number.isFinite(maxTokens) && ctxSize > 0) {
        if (maxTokens >= ctxSize) {
            notices.push(`「单次最大输出 tokens」（${maxTokens}）不小于「上下文长度」（${ctxSize}）：` +
            ...
            notices.push(`「单次最大输出 tokens」（${maxTokens}）超过了上下文长度的一半（${ctxSize}）：` +
    ...
    if (kvmemGenReserve >= 0 && Number.isFinite(maxTokens) && maxTokens > kvmemGenReserve) {
        notices.push(`「解码预留」（--kvmem-gen-reserve = ${kvmemGenReserve}）小于「单次最大输出 tokens」（${maxTokens}）：` +
