 ...
 */
export const DEFAULT_GUARD_LIMITS = {
    floor: 512,
...
export function isContextOverflowError(text) {
    return /exceeds\s+n_ctx|exceeds\s+the\s+available\s+context|prompt\s+is\s+too\s+long|prompt\s+too\s+long/i.test(text);
}
/** 请求体里的输出上限；没有该字段时返回 null（= 由服务端的 -n 决定）。 */
export function readMaxTokens(body) {
...
export function writeMaxTokens(body, value) {
    for (const field of MAX_TOKENS_FIELDS) {
 ...
 */
export function preflightMaxTokens(raw, limits) {
    const source = Buffer.isBuffer(raw) ? raw : Buffer.from(raw, 'utf8');
    const unchanged = { body: source, changed: false, from: null, to: null };
    let payload;
    try {
    ...
    if (from === null || !Number.isFinite(limits.nCtx) || limits.nCtx <= 0)
        return unchanged;
    if (from < limits.nCtx)
        return unchanged;
 ...
 */
export function nextRetryMaxTokens(current, limits) {
    if (!Number.isFinite(limits.nCtx) || limits.nCtx <= 0)
        return null;
...
export function applyMaxTokens(raw, value) {
    const source = Buffer.isBuffer(raw) ? raw : Buffer.from(raw, 'utf8');
    let payload;
    try {
        payload = JSON.parse(source.toString('utf8'));
 ...
 */
export function overflowHint(nCtx, promptTokens) {
    const parts = [
