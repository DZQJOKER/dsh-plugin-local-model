...
import { scanModelsDetailed, pickModel, formatBytes, effectiveVisionProjector, resolveVisionProjector, } from './registry.js';
import { ensureReadme } from './paths.js';
import { buildLlamaServerArgs, DEFAULT_ALIAS } from './llama/args.js';
import { buildLaunchReport } from './launchReport.js';
...
import { isFlashAttnFormError, probeCapabilities, unknownFlags, } from './llama/capabilities.js';
import { diagnoseLoadFailure, extractEffectiveContext } from './llama/diagnose.js';
import { fetchServerChatTemplate, fetchServerContext, findFreePort, killStaleLlamaProcess, LlamaServer, } from './llama/runner.js';
...
export function shouldUnload(input) {
    if (input.state !== 'ready')
        return false;
    if (!Number.isFinite(input.idleUnloadMs) || input.idleUnloadMs <= 0)
        return false;
    const idleFor = input.now - input.lastActivityAt;
    if (idleFor < input.idleUnloadMs)
        return false;
 ...
 */
export function buildArgInput(config, entry, options) {
    return {
...
/** 默认启动方式：拼参数 → 起真进程。 */
export function defaultServerLauncher(input) {
    const { config, entry, port, executable, log, onExit, flashAttnMode, gpuLayersSupport, knownFlags } = input;
    const built = buildLlamaServerArgs(buildArgInput(config, entry, { port, flashAttnMode, gpuLayersSupport, knownFlags }));
    /**
...
export class LocalModelRuntime {
     ...
     */
    async applyConfig(next, options = {}) {
    ...
    async bootstrap() {
        ...
        try {
            ...
            this.log.debug(`清理残留进程时出错：${error.message}`);
        }
        // 常驻代理很轻（不加载模型），但它让端点「永远在线」—— 这正是首条对话能触发加载的前提。
        if (!this.proxy)
            throw new Error('代理未注入');
        ...
        if (this.config.preload && this.selectedModelId) {
            ...
            void this.ensureReady().catch((error) => this.log.warn(`预加载失败：${error.message}`));
        }
    }
    async dispose() {
        this.disposed = true;
    ...
    async ensureReady() {
        ...
        if (!this.config.enabled)
            throw new Error('本地模型插件已禁用。请在 Harness 设置 → 本地模型 中打开总开关。');
        for (;;) {
            if (this.stopPromise) {
    ...
    async reload(reason = '手动重新加载') {
        await this.unload(reason);
        await this.ensureReady();
    }
    async refreshModels(force = false) {
        const now = Date.now();
        if (!force && now - this.modelsScannedAt < SCAN_TTL_MS)
            return this.models;
        try {
            ...
            this.log.error(`扫描模型目录失败：${error.message}`);
            this.models = [];
     ...
     */
    effectiveVisionProjector(entry = pickModel(this.models, this.selectedModelId)) {
        if (!entry)
            return null;
    ...
    async selectModel(id) {
        ...
        if (!entry) {
            ...
            throw new Error(`模型目录里找不到「${id}」。当前可用（${this.models.length}）：\n${available.map((m) => `  - ${m}`).join('\n') || '  （目录为空）'}`);
        }
        if (!entry.complete) {
     ...
     */
    explainUnloadBlocked(idleMs) {
        if (idleMs <= 0)
            return '空闲卸载已关闭（idleUnloadMinutes = 0）';
        if (this.state !== 'ready') {
            if (this.lastError)
                return `状态 ${this.state}，加载未成功（${this.lastError}）`;
            return `状态 ${this.state}，模型未就绪`;
        }
        if (this.activeRequests > 0)
            return `还有 ${this.activeRequests} 个进行中请求（finish() 漏调？）`;
        const remaining = idleMs - (Date.now() - this.lastActivityAt);
        if (remaining > 0)
            return `还有 ${(remaining / 60_000).toFixed(1)} 分钟到期`;
        return null;
    }
    ...
    describe() {
        ...
        if (status.upstream)
            lines.push(`上游：${status.upstream}${status.pid ? `（pid ${status.pid}）` : ''}`);
        if (status.visionProjector)
            lines.push(`视觉投影：${status.visionProjector}`);
        lines.push(status.reasoningEfforts
            ? `模型支持的推理档位：${status.reasoningEfforts.join(' / ')}（对话框里的档位会按它重映射）`
            : '模型支持的推理档位：未解析出（本次不下发档位，只按开关控制思考与否）');
        if (status.state === 'ready') {
            lines.push(this.config.idleUnloadMinutes > 0
                ? `空闲卸载：连续 ${this.config.idleUnloadMinutes} 分钟无交互即释放${status.unloadAt ? `（约 ${new Date(status.unloadAt).toLocaleTimeString()}）` : ''}`
                : '空闲卸载：已关闭');
        }
        if (status.activeRequests > 0)
            lines.push(`进行中请求：${status.activeRequests}`);
        if (status.modelsFound === 0)
            lines.push(`模型目录为空：${status.modelsDir}`);
        if (status.lastError)
    ...
    // ── 内部实现 ────────────────────────────────────────────────────────────
    upstreamOrigin() {
        if (!this.server?.running || !this.upstreamPort)
            return null;
    ...
    async doStart() {
        ...
        try {
            ...
            if (!location) {
                throw new Error('没有找到 llama-server 可执行文件。请下载 llama.cpp 并解压到运行时目录，或在设置里填写绝对路径。\n已查找位置：\n' +
            ...
            if (!capabilities.ok) {
                this.log.warn(`无法探测 llama-server 的选项支持情况（${capabilities.detail}）。Flash Attention 将不下发，交由 llama.cpp 的默认值决定。`);
            }
            else {
            ...
            const attempts = preferred === 'unsupported' ? ['unsupported'] : [preferred, 'unsupported'];
            // 配置一致性提醒：与 flash-attn 的两种尝试无关，所以放在循环外面只打一次。
            // 这些数字各自写在不同的地方（dsh 路由声明 / 启动参数 / kvmem 解码预留），
            // 对不上时加载阶段一声不响，要等到某次对话才以一条读不懂的服务端错误爆出来。
            for (const notice of consistencyNotices(this.config))
                this.log.warn(`配置提醒：${notice}`);
            let server = null;
            let failure = null;
            /**
            ...
            for (const mode of attempts) {
                const built = buildLlamaServerArgs(buildArgInput(this.config, entry, {
                    port,
                    flashAttnMode: mode,
                ...
                for (const notice of built.notices)
                    this.log.warn(`参数提示：${notice}`);
                if (capabilities.ok) {
                    const unknown = unknownFlags(built.usedFlags, capabilities.flags);
                    if (unknown.length > 0) {
                        this.log.warn(`这个 llama-server 不认识以下选项：${unknown.join('、')}；加载可能失败。` +
                ...
                this.log.info(`正在加载 ${entry.displayName}（${formatBytes(entry.sizeBytes)}，可执行文件来自 ${location.source}）`);
                try {
                    ...
                    await candidate.stop(this.config.shutdownGraceMs).catch(() => undefined);
                    throw new Error(explainServerFailure(tail, error, this.config));
                }
            }
            if (!server)
                throw failure ?? new Error('无法启动 llama-server');
            this.server = server;
    ...
    async doUnload(reason) {
        ...
        this.restarts = 0;
        if (hadServer)
            this.log.info(`已卸载本地模型（${reason}），显存与内存已释放`);
        this.touch();
        this.setState(this.config.enabled ? 'idle' : 'disabled');
    }
    async stopServerQuietly() {
        ...
        try {
            ...
            this.log.warn(`停止 llama-server 时出错：${error.message}`);
        }
    }
    handleExit(info) {
        if (this.disposed)
    ...
    async resolveSelectedModel() {
        await this.refreshModels(true);
        const id = this.selectedModelId;
        if (!id) {
            const available = this.models.map((m) => m.id).slice(0, 20);
            throw new Error(`还没有选择本地模型。\n请把 GGUF 放进 ${this.config.modelsDir}，然后在 Harness 设置 → 本地模型 的「模型」下拉框中选择。\n` +
                (available.length ? `当前可用：\n${available.map((m) => `  - ${m}`).join('\n')}` : '（模型目录当前为空）'));
        }
        const entry = pickModel(this.models, id);
        if (!entry) {
            throw new Error(`选中的模型「${id}」已不在模型目录里（可能被移动或删除）。当前可用：${this.models.map((m) => m.id).join('、') || '（空）'}`);
        }
        if (!entry.complete) {
            throw new Error(`模型「${entry.displayName}」分片不完整，缺少：${entry.missing.join('、') || '（未知）'}`);
        }
    ...
    async reconcileContext(port) {
        ...
        if (effective < declared) {
            this.log.warn(`llama.cpp 实际生效的上下文是 ${effective}，小于你设置的上下文长度 ${declared}。` +
     ...
     * 少一层颗粒度，好过一开口就失败。
     */
    async discoverEffortVocabulary(port) {
        if (this.disposed)
            return;
        ...
        if (efforts) {
            this.log.info(`模型模板支持的推理档位：${efforts.join(' / ')}。` +
                '对话框里的档位会按它重新映射（模板对不认识的档位会直接报错，所以不能原样透传）。');
        }
        else {
            this.log.info('未能从模型模板解析出推理档位表。本次只按开关控制「思考与否」，不下发具体档位 —— ' +
                '宁可少一层颗粒度，也不冒模板报错的风险。');
        }
    }
    armTicker() {
    ...
    tick() {
        ...
        void this.unload(`空闲 ${minutes} 分钟无对话交互`).catch((error) => {
            ...
            this.log.warn(`空闲卸载失败（${error.message}），下一轮 tick 会重试`);
        });
    }
    setState(state) {
        this.state = state;
 ...
 */
function explainServerFailure(logTail, original, config) {
    ...
    if (diagnosis) {
        ...
        if (diagnosis.evidence)
            lines.push(`（判据：${diagnosis.evidence}）`);
        if (logTail.trim()) {
