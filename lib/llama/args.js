...
export function buildLlamaServerArgs(input) {
    ...
    if (input.threads > 0)
        gateLoose('-t', String(input.threads));
    if (input.threadsBatch > 0)
        gateLoose('--threads-batch', String(input.threadsBatch));
    if (input.batchSize > 0)
        gateLoose('-b', String(input.batchSize));
    ...
    if (cacheK !== 'auto')
        gateLoose('--cache-type-k', cacheK);
    const cacheV = normalizeCacheType(input.cacheTypeV);
    if (cacheV !== 'auto')
        gateLoose('--cache-type-v', cacheV);
    // ── KV 缓存策略 ──────────────────────────────────────────────────────────
    /**
     * 只在构建公开了这个选项时才下发。
     *
    ...
    if (imageMin > 0)
        gateStrict('--image-min-tokens', String(imageMin));
    if (imageMax > 0)
        gateStrict('--image-max-tokens', String(imageMax));
    // ── 推理预算 ────────────────────────────────────────────────────────────
    ...
    if (input.chatTemplate.trim())
        gateLoose('--chat-template', input.chatTemplate.trim());
    /**
    ...
    if (input.mtp) {
        ...
        if (mmproj) {
            ...
            if (input.mtpWithVision !== false) {
                ...
                notices.push('已同时下发 --spec-type draft-mtp 与 --mmproj（「MTP 与视觉共存」为开）。' +
                    'kvmem 分支的 llama-kvmem-server 实测支持这种组合；' +
                    '若你换回官方 llama.cpp 并在加载时看到错误，把「MTP 与视觉共存」关掉即可恢复互斥。');
            }
            else {
                notices.push('已开启 MTP，视觉投影文件（--mmproj）被忽略：官方 llama.cpp 的 MTP 与图像输入不能同时下发。' +
    ...
    if (input.apiKey.trim())
        gateLoose('--api-key', input.apiKey.trim());
    // ── 输出上限与加载方式 ───────────────────────────────────────────────────
    /**
     * 数值选项的统一入口：**负数一律不下发**。
     *
    ...
    if (input.kvmemMethod.trim())
        gateStrict('--kvmem-method', input.kvmemMethod.trim());
    ...
    if (input.kvmemQueryReplay.trim())
        gateStrict('--kvmem-query-replay', input.kvmemQueryReplay.trim());
    if (input.kvmemQueryPolicy.trim())
        gateStrict('--kvmem-query-policy', input.kvmemQueryPolicy.trim());
    if (input.kvmemMtpState.trim())
        gateStrict('--kvmem-mtp-state', input.kvmemMtpState.trim());
    ...
    if (input.kvmemNvmeDir.trim())
        gateStrict('--kvmem-nvme-dir', input.kvmemNvmeDir.trim());
    if (input.kvmemHarvestV)
        gateStrict('--kvmem-harvest-v');
    if (input.kvmemRawKNvme)
        gateStrict('--kvmem-raw-k-nvme');
    /**
    ...
    if (genReserveSupported) {
        ...
        if (effective < 1024) {
            notices.push(`解码预留（--kvmem-gen-reserve）当前为 ${effective}，而它是**单次生成的上限**（含思考）—— ` +
                '模型每次回复最多只能写这么多 token，长回答会被硬截断且不报错。' +
                '建议设成不小于 dsh 路由的「单次最大输出 tokens」（例如 8192～16384）。');
        }
    }
    // ── KV 类型与 MTP 细节 ───────────────────────────────────────────────────
    if (input.kvDtype.trim())
        gateStrict('--kv-dtype', input.kvDtype.trim());
    if (input.specKvDtype.trim())
        gateLoose('--spec-kv-dtype', input.specKvDtype.trim());
    ...
    if (input.kvDtype.trim() && (normalizeCacheType(input.cacheTypeK) !== 'auto' || normalizeCacheType(input.cacheTypeV) !== 'auto')) {
        notices.push('「KV 缓存类型（合并）」与「KV cache 精度（K/V）」同时设置了：前者对应 --kv-dtype、' +
            '后者对应 -ctk/-ctv，两者作用重叠，生效顺序由构建内部决定。建议只留一个。');
    }
    // ── 模板、推理档位与其它 ─────────────────────────────────────────────────
    if (!input.mmprojOffload)
        gateLoose('--no-mmproj-offload');
    if (input.chatTemplateFile.trim())
        gateLoose('--chat-template-file', input.chatTemplateFile.trim());
    if (input.chatTemplateKwargs.trim())
        gateLoose('--chat-template-kwargs', input.chatTemplateKwargs.trim());
    if (input.reasoningEffort.trim())
        gateLoose('--reasoning-effort', input.reasoningEffort.trim());
    if (input.reasoningBudgetMessage.trim())
        gateLoose('--reasoning-budget-message', input.reasoningBudgetMessage.trim());
