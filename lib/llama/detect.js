...
export function llamaBinaryNames(platform = process.platform) {
    if (platform === 'win32')
...
export async function locateLlamaServer(opts) {
    ...
    if (explicit) {
        ...
        for (const candidate of candidates) {
            if (await isExecutableFile(candidate))
                return { path: candidate, source: 'config' };
        }
    }
    ...
    for (const root of roots) {
        for (const name of names) {
            ...
            if (await isExecutableFile(candidate))
                return { path: candidate, source: 'runtime-dir' };
        }
    }
    ...
    for (const segment of pathEnv.split(path.delimiter)) {
        ...
        for (const name of names) {
            ...
            if (await isExecutableFile(candidate))
                return { path: candidate, source: 'path-env' };
        }
    }
    return null;
}
/** 供错误信息使用：告诉用户到底在哪里找过了。 */
export function describeSearchScope(opts) {
