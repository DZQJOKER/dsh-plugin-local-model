...
export async function findFreePort(host, preferred) {
    if (preferred > 0 && (await isPortFree(host, preferred)))
        return preferred;
...
async function terminateForcefully(pid) {
    try {
        if (process.platform === 'win32')
...
export function probeHealth(host, port, timeoutMs = 2000) {
    return new Promise((resolve) => {
 ...
 */
export function fetchServerChatTemplate(host, port, timeoutMs = 3000) {
    return new Promise((resolve) => {
...
export function fetchServerContext(host, port, timeoutMs = 3000) {
    return new Promise((resolve) => {
        const req = http.request({ host, port, path: '/props', method: 'GET', timeout: timeoutMs }, (res) => {
            const chunks = [];
            res.on('data', (c) => chunks.push(c));
...
export class LlamaServer {
    ...
    async start() {
        ...
        child.once('exit', (code, signal) => {
            ...
            if (expected)
                this.options.log.info(`llama-server 已退出（code=${code}）`);
            else
                this.options.log.error(`llama-server 意外退出（code=${code} signal=${signal}）`);
    ...
    async waitUntilReady(host, port, timeoutMs, signal) {
        const deadline = Date.now() + timeoutMs;
        let lastDetail = '';
        let polls = 0;
        while (Date.now() < deadline) {
            if (signal?.aborted)
                throw new Error('启动被取消');
        ...
        throw new Error(`等待模型加载超时（${timeoutMs}ms）。最后一次探测：${lastDetail}\n最近日志：\n${this.tail.slice(-15).join('\n')}`);
    }
    /**
    ...
    append(line) {
        this.tail.push(line);
        if (this.tail.length > LOG_RING)
            this.tail.splice(0, this.tail.length - LOG_RING);
    ...
    async writePidFile(pid) {
        try {
            ...
            this.options.log.debug(`写 pid 文件失败：${error.message}`);
        }
    }
    async clearPidFile() {
        try {
...
async function readPidFileRaw(pidFile) {
    try {
 ...
 */
export async function killStaleLlamaProcess(pidFile, log) {
