...
export function parseParams(fileName) {
 ...
 */
export function groupShards(shards, mmprojs = []) {
    ...
    for (const shard of shards) {
        ...
        const entry = groups.get(key) ?? { total: Number(m.groups.total), byIndex: new Map() };
    ...
    for (const group of groups.values()) {
        ...
        for (let i = 1; i <= group.total; i++) {
            if (!group.byIndex.has(i))
                missing.push(`-${String(i).padStart(5, '0')}-of-${String(group.total).padStart(5, '0')}.gguf`);
        }
...
function buildEntry(shards, mmprojs, modelsInDir) {
 ...
 */
function matchMmproj(modelRel, mmprojs, modelsInDir) {
    ...
    if (best && bestScore >= MIN_MMPROJ_PREFIX)
        return best.abs;
    if (sameDir.length === 1 && modelsInDir === 1)
        return sameDir[0].abs;
    return null;
}
...
export function pickModel(entries, selected) {
    const wanted = (selected ?? '').trim().replace(/\\/g, '/');
    if (!wanted)
        return null;
    const lower = wanted.toLowerCase();
    return (entries.find((e) => e.id === wanted) ??
 ...
 */
export async function scanModelsDetailed(modelsDir) {
    const shards = [];
    const mmprojs = [];
    async function walk(dir, depth) {
        ...
        for (const dirent of dirents) {
            ...
            if (!dirent.isFile() || !/\.gguf$/i.test(dirent.name))
                continue;
            let info;
            try {
                info = await stat(abs);
            ...
            if (isMmproj(dirent.name))
                mmprojs.push(shard);
            else if (isShard(dirent.name))
                shards.push(shard);
        }
    }
...
export async function scanModels(modelsDir) {
 ...
 */
export function resolveVisionProjector(modelsDir, selected, fallback) {
...
export function effectiveVisionProjector(options) {
    if (options.mtp && options.mtpWithVision === false)
        return '';
