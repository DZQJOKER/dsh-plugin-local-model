...
const GROUP_DEFS = [
    ...
    },
    {
        ...
        hint: '只在上面「模型与目录」里打开了「多 Token 预测（MTP）」时才起作用。' +
    ...
    },
    {
        id: 'sampling',
        title: '采样与 KV 缓存',
        hint: '采样参数会显式下发给 llama-server 并覆盖它自己的默认值（本组默认值与 llama.cpp 默认值并不相同，表现在各项说明里）。' +
    ...
    },
    {
        ...
        hint: '整组都只对 llama-kvmem-server 生效，官方 llama.cpp 构建会全部跳过（探测不到即不下发）。' +
