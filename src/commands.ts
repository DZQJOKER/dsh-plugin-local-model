...
interface CommandRegistry {
  register(command: unknown): unknown
}
 ...
 */
export function registerLocalModelCommands(
  ctx: Context,
  runtime: LocalModelRuntime,
  log: Log,
  routeYaml: () => string,
): () => void {
  const commands = ctx.get<CommandRegistry>('commands')
  ...
  const run = async (input: unknown): Promise<string> => {
    ...
    switch (action) {
      ...
      case 'list': {
        ...
        if (models.length === 0) return `模型目录为空：${runtime.config.modelsDir}`
        return models
          .map(
            (m) =>
  ...
  try {
    ...
    log.debug(`注册 /local-model 命令失败：${(error as Error).message}`)
    return () => undefined
  }
}
...
function extractAction(input: unknown): string | undefined {
  ...
  if (input && typeof input === 'object') {
    ...
    for (const key of ['action', 'args', 'argv', 'arguments', 'input']) {
      ...
      if (typeof value === 'string') return value.trim().split(/\s+/)[0]
...
function normalize(value: string | undefined): string {
  ...
  if (!raw || raw === 'local-model' || raw === 'localmodel') return 'status'
  return raw
}
