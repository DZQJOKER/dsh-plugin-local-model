...
export function apply(ctx: Context, config?: Partial<LocalModelConfig>): void {
  ...
  const runtime = new LocalModelRuntime(boot, log, {
    persistSelectedModel: (id) => {
      void store.update({ selectedModel: id }).catch((error) => {
        log.warn(`保存所选模型失败：${(error as Error).message}`)
      })
  ...
  registerLocalModelCommands(ctx, runtime, log, () =>
    renderRouteYaml(buildRouteSpec(runtime.config, `${proxy.origin}/v1`)),
  )

  void registerLocalModelTool(ctx, runtime, log).catch((error) => {
    log.warn(`注册 local_model 工具失败：${(error as Error).message}`)
  })

  log.info(
    `已加载 v${PLUGIN_VERSION}。模型目录 ${boot.paths.modelsDir}；空闲卸载 ${
      boot.idleUnloadMinutes > 0 ? `${boot.idleUnloadMinutes} 分钟` : '已关闭'
    }；入口端口 ${boot.port}`,
  )
}
