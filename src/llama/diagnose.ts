...
const RULES: Rule[] = [
  ...
  },
  {
    ...
    build: (log) => {
      ...
      return {
        summary: `显存不够，而 llama.cpp 的自动适配被「手动指定的 GPU 层数」拦住了${
          pinned ? `（当前 ${pinned} 层）` : ''
        }。`,
        actions: [
...
export function diagnoseLoadFailure(logTail: string, context: { gpuLayersPinned?: boolean } = {}): Diagnosis | null {
  const log = logTail ?? ''
  if (!log.trim()) return null

  for (const rule of RULES) {
    if (!rule.test.test(log)) continue
 ...
 */
export function extractEffectiveContext(logTail: string): number | null {
