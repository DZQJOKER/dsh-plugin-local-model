...
const TOOL_NAMES = ['ffprobe', 'ffmpeg'] as const
...
function exeName(base: string, platform: NodeJS.Platform): string {
  return platform === 'win32' ? `${base}.exe` : base
}

/** 某个目录里是否两个工具都在。 */
function completeIn(dir: string, platform: NodeJS.Platform): boolean {
 ...
 * 那种「半套工具」会让人以为已经修好了。
 */
export function findMediaTools(
  options: { env?: NodeJS.ProcessEnv; platform?: NodeJS.Platform; extraDirs?: (string | undefined)[] } = {},
): MediaTools {
  ...
  for (const candidate of candidates) {
    ...
    seen.add(dir)
    try {
      if (!completeIn(dir, platform)) continue
    } catch {
      continue
    }
 ...
 */
export function mediaToolWarning(): string {
  return (
    '已加载视觉投影文件，但本机找不到 ffmpeg / ffprobe。' +
