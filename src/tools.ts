...
export async function registerLocalModelTool(
  ...
  let defineTool: (definition: unknown) => unknown
  try {
    const mod = (await import('@deepseek-ai/dsh-tools')) as { defineTool?: (definition: unknown) => unknown }
    if (typeof mod.defineTool !== 'function') throw new Error('模块没有导出 defineTool')
    defineTool = mod.defineTool
  } catch (error) {
    log.warn(`无法加载 @deepseek-ai/dsh-tools（${(error as Error).message}），跳过 local_model 工具注册`)
    return () => undefined
  }
  ...
  const tool = defineTool({
    name: 'local_model',
    description:
      '查询和操作本机的本地大模型（llama.cpp）。action=status 看当前是否已加载/何时自动卸载；' +
    ...
    output: {
      ...
      render: (_args: unknown, value: unknown) => {
        const payload = value as { message?: string }
        return [{ type: 'text', text: payload?.message ?? JSON.stringify(value, null, 2) }]
      },
    },
    async execute(args: { action?: string; model?: string }, exec: { signal: AbortSignal }) {
      ...
      switch (action) {
            ...
            message:
              models.length === 0
                ...
                : models
                    .map(
                      (m) =>
                        `${m.id === runtime.selectedModelId ? '* ' : '  '}${m.displayName}` +
        ...
        case 'start': {
          if (args?.model) await runtime.selectModel(args.model)
...
function normalizeAction(value: string | undefined, allowed: ToolAction[]): ToolAction | null {
  const raw = (value ?? '').trim().toLowerCase()
  return (allowed as string[]).includes(raw) ? (raw as ToolAction) : null
}
